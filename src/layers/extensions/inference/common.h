...
// =========================================================================
// [HYNX FIX V2] Smart Operator Overloads
// We use 'decltype' to ensure these operators ONLY match types that actually
// have .x, .y, .z, .w members. This prevents conflicts with std::atomic.
// =========================================================================
...
// [KEY FIX] Replaced the generic template with a SFINAE-guarded one.
// The 'decltype' checks if 'a.x' is valid. If not (like std::atomic), this function is ignored.
template <typename T1, typename T2>
__forceinline__ __device__ auto operator>(const T1& a, const T2& b) -> decltype(a.x, a.y, a.z, a.w, bool4())
{
    return make_vec4(a.x > b, a.y > b, a.z > b, a.w > b);
}
