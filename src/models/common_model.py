...
class CompressionModel(nn.Module):
    ...
    # --- Step 0 ---
    # 计算似然值 (使用 GaussianConditional 内部的估计方法)
    # y_group_0 = y * mask_0, means_0 = means * mask_0, scales_0 = scales * mask_0
        y_hat_0, likelihood_0 = self.gaussian_conditional(y, scales, means)
        y_hat_0 = y_hat_0 * mask_0
        y_likelihoods_list.append(likelihood_0 * mask_0)

    # --- Step 1 ---
        y_hat_so_far = y_hat_0
        params = torch.cat((y_hat_so_far, common_params), dim=1)
        scales, means = y_spatial_prior(y_spatial_prior_adaptor_1(params)).chunk(2, 1)
        y_hat_1, likelihood_1 = self.gaussian_conditional(y, scales, means)
        y_hat_1 = y_hat_1 * mask_1
        y_likelihoods_list.append(likelihood_1 * mask_1)

    # --- Step 2 ---
        y_hat_so_far = y_hat_so_far + y_hat_1
        params = torch.cat((y_hat_so_far, common_params), dim=1)
        scales, means = y_spatial_prior(y_spatial_prior_adaptor_2(params)).chunk(2, 1)
        y_hat_2, likelihood_2 = self.gaussian_conditional(y, scales, means)
        y_hat_2 = y_hat_2 * mask_2
        y_likelihoods_list.append(likelihood_2 * mask_2)

    # --- Step 3 ---
        y_hat_so_far = y_hat_so_far + y_hat_2
        params = torch.cat((y_hat_so_far, common_params), dim=1)
        scales, means = y_spatial_prior(y_spatial_prior_adaptor_3(params)).chunk(2, 1)
        y_hat_3, likelihood_3 = self.gaussian_conditional(y, scales, means)
        y_hat_3 = y_hat_3 * mask_3
        y_likelihoods_list.append(likelihood_3 * mask_3)

    # 合并重建结果和似然值
        y_hat = (y_hat_so_far + y_hat_3) * q_dec
    
    # 似然值相加（因为带了 mask，相加等同于拼回原图大小）
        y_likelihoods = y_likelihoods_list[0] + y_likelihoods_list[1] + \
                     y_likelihoods_list[2] + y_likelihoods_list[3]
